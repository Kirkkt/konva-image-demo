...
const ImageSources = [...new Array(STAGES).keys()].map(key => require('./assets/400x200-' + key + '.jpeg'))

const getNewLayers = () => [...new Array(STAGES).keys()];
const getNewImageData = () => [...new Array(LAYERS).keys()].map(stageKey => [...new Array(STAGES).keys()].map(imageKey => ({
  opacity: Math.random(),
  ...
  render() {
    return (
      <div>
        ...
        {[...new Array(LAYERS).keys()].map(stageKey => (
